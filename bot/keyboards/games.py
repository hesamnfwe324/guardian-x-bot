...
def join_game_kb(_: Callable, match_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="▶️ Join Game", callback_data=f"game_join:{match_id}")
...
def wheel_spin_kb(_: Callable, match_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🎡 SPIN!", callback_data=f"wheel_spin:{match_id}")
    builder.adjust(1)
    return builder.as_markup()


def mines_board_kb(board: list[list[bool | None]], match_id: int, rows: int = 5, cols: int = 5) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for r in range(rows):
        for c in range(cols):
